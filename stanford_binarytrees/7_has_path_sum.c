...
void has_path_sum(struct tnode *root, int sum, int *path, int level);
...
int main(int argc, int *argv[]) 
{
	...
	scanf("%d", &sum);
    has_path_sum(pnode, sum, path, level);
...
void has_path_sum(struct tnode *root, int sum, int *path, int level) 
{
	...
	if (tsum == 0) 
	    print_array(path, i+1, level);

    has_path_sum(root->left, sum, path, level+1);

    has_path_sum(root->right, sum, path, level+1);
